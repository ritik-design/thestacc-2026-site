#!/usr/bin/env python3
"""Migrate small-chunks/chunk-006 to the 2026 blog design."""
...
CHUNK_FILE = os.path.join(ROOT, "Stacc/blog-migration/small-chunks/chunk-006.txt")
PROGRESS_FILE = os.path.join(ROOT, "Stacc/blog-migration/progress.json")
CHUNK_PROGRESS_FILE = os.path.join(ROOT, "Stacc/blog-migration/small-chunks/chunk-006.txt.progress.json")
...
CATEGORY_KEYWORDS = [
    ("AI & Emerging", ["ai", "agent", "agents", "chatgpt", "gpt", "llm", "generative", "gemini", "perplexity", "claude", "copilot", "machine learning", "algorithm update", "geo", "generative-engine", "ai search", "ai-overview", "ai mode", "mcp", "model context protocol"]),
    ("Local SEO", ["local seo", "local business", "google business profile", "gbp", "map pack", "near me", "citation", "dentist", "lawyer", "contractor", "realtor", "salon", "florist", "electrician", "funeral", "gym", "hvac", "chiropractor", "accountant", "home service", "restaurant", "catering", "city service", "online presence", "med spa", "moving company", "music school", "nap consistency", "multi-location", "multi location"]),
...
def classify_author(title, slug, category):
    text = (title + " " + slug.replace("-", " ")).lower()
    sidd = ["ai", "agent", "chatgpt", "llm", "generative", "gemini", "perplexity", "claude", "copilot", "algorithm", "technical", "architecture", "founder", "product strategy", "machine learning", "mcp", "model context protocol"]
    ritik = ["growth", "programmatic", "analytics", "ga4", "search console", "conversion", "funnel", "ab test", "a/b test", "roi", "statistics", "case study", "data", "tool comparison"]
    def word_match(k):
        return bool(re.search(r"\b" + re.escape(k) + r"\b", text))
    s = sum(1 for k in sidd if word_match(k))
    r = sum(1 for k in ritik if word_match(k))
...
def parse_source(html_text, slug=""):
    ...
    # Override source category for posts whose topic clearly belongs elsewhere.
    combined = (h1 + " " + slug.replace("-", " ")).lower()
    if "mcp" in combined or "model context protocol" in combined:
        category = "AI & Emerging"
    elif "copilot" in combined and ("search" in combined or "seo" in combined):
        category = "AI & Emerging"
    elif "multi-agent" in combined:
        category = "AI & Emerging"
    elif "ai-marketing" in slug or "ai-content" in slug:
        category = "AI & Emerging"
    sections = []
    ...
    if col:
        ...
        for h in headings:
            level = int(h.name[1])
            title = clean_section_title(clean_text(h.get_text()))
...
def optimize_title(h1, slug):
    ...
    if len(t) > 65:
        ...
        # drop year markers like (2026) to save space
        prefix = re.sub(r"\s*\(\d{4}\)\s*", " ", prefix).strip()
        prefix = re.sub(r"\s+:", ":", prefix)
        # remove trailing fragment like ": Strategies, Tactics & Examples" if too long
        max_prefix = 56
...
def main():
    ...
    for slug in slugs:
        ...
        if err:
            ...
            continue
        try:
            data = parse_source(html_text, slug)
