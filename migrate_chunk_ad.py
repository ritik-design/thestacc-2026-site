#!/usr/bin/env python3
"""Bulk migration script for chunk-ad blog slugs."""
...
CHUNK_FILE = os.path.join(ROOT, "Stacc/blog-migration/chunks/chunk-ad")
PROGRESS_FILE = os.path.join(ROOT, "Stacc/blog-migration/progress.json")
CHUNK_PROGRESS_FILE = os.path.join(ROOT, "Stacc/blog-migration/chunks/chunk-ad-progress.json")
...
CATEGORY_KEYWORDS = [
    ...
    ("Local SEO", ["local seo", "local business", "google business profile", "gbp", "map pack", "near me", "citation", "dentist", "lawyer", "contractor", "realtor", "salon", "florist", "electrician", "funeral", "gym", "hvac", "chiropractor", "accountant", "home service", "restaurant"]),
    ("Content Strategy", ["content strategy", "content marketing", "editorial", "calendar", "blog post", "writing", "brief", "newsletter", "email marketing", "email automation", "subject line", "segmentation", "ugc", "evergreen", "format", "fact-check", "eeat", "e-e-a-t"]),
]
...
def classify_author(title, slug, category):
    ...
    ritik = ["growth", "programmatic", "cro", "analytics", "ga4", "search console", "conversion", "funnel", "ab test", "a/b test", "roi", "statistics", "case study", "data"]
...
def js_str(s):
    # Return a JSON string suitable for embedding in JS frontmatter.
    return json.dumps(s, ensure_ascii=False)

def escape_braces(s):
    # Escape literal braces in body HTML so Astro doesn't parse them as expressions.
    return s.replace("{", "&#123;").replace("}", "&#125;")

ALLOWED_TAGS = {"p", "ul", "ol", "li", "pre", "code", "table", "thead", "tbody", "tr", "th", "td", "strong", "em", "br", "span", "h3", "h4"}

def clean_element(el):
    # Remove disallowed tags, scripts, styles, images, iframes. Strip attributes except for code language maybe.
...
def parse_source(html_text):
    ...
    if not col:
        # fallback: largest div inside article/main
        candidates = []
    ...
    faq_items = []
    if col:
        # remove explore more / newsletter / cta sections at end
        for h2 in col.find_all("h2"):
            txt = h2.get_text(strip=True).lower()
            if any(x in txt for x in ["explore more", "stop writing", "get your free", "check your inbox", "subscribe", "newsletter"]):
                for sib in list(h2.find_all_next()):
                    sib.decompose()
                h2.decompose()
        headings = col.find_all(["h2", "h3"])
        # extract FAQ if h2 == FAQ
        for idx, h in enumerate(headings):
            if re.search(r"\bfaq\b", h.get_text(strip=True), re.I):
                ...
                h.decompose()
                break
        # rebuild headings list after faq removal
        headings = col.find_all(["h2", "h3"])
        for h in headings:
            level = int(h.name[1])
...
def make_feature_svg(title, slug):
    # Return an inline SVG themed to the post. Keep it abstract but on-brand.
    color1 = "#4f39f6"
    color2 = "#818cf8"
    bg = "#f5f3ff"
    topic = title[:30]
    # simple deterministic shapes based on slug length
    shapes = [
...
def internal_links_for(category, slug):
    links = []
    checkout = '<a href="/checkout/">Start your $1 trial</a>'
    if category == "AI & Emerging":
        links = [
...
def external_sources_for(category, slug):
    common = [
        ...
        ("https://ahrefs.com/blog/", "Ahrefs Blog — SEO research and tactics"),
    ]
    if category == "AI & Emerging":
        return [
...
def build_sections_html(sections, category, slug, h1):
    # Build article body from extracted sections.
    # We inject our own internal links contextually near the top.
    ...
    def safe_id(title):
        ...
        seen_ids.add(uniq)
        return uniq
    # First few sections: keep as is, but ensure h2 id
    for sec in sections:
        if sec["level"] == 2:
        ...
        body = sec["body"]
        if body:
            # ensure no banned words (simple replace)
    ...
    # Inject contextual internal link paragraph near top if short
    if len(parts) > 2 and links:
        insert_idx = 2
        parts.insert(insert_idx, f'<p>If you want to go deeper, explore our {links[0]} or {links[1]}.</p>')
...
def build_page(slug, data, progress):
    ...
    # lede from first section or generated
    ...
    # build toc from h2 ids
    toc_items = []
    for hid in h2_ids:
        label = hid.replace("-", " ").title()
        toc_items.append(f'<li><a href="#{hid}">{html.escape(label)}</a></li>')
    # ensure key sections present
    for sec, label in [("common-mistakes", "Common mistakes"), ("faq", "FAQ"), ("sources", "Sources")]:
        if sec not in h2_ids and sec != "faq" and sec != "sources":
            pass
    # Add static toc entries for FAQ and Sources if not present as h2
