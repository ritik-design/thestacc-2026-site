#!/usr/bin/env python3
"""Migrate chunk-016 blog slugs to the 2026 theStacc design."""
import json, os, re, subprocess, textwrap, html
from datetime import datetime, timezone
from bs4 import BeautifulSoup

ROOT = "/home/ritik/thestacc.com-astro/thestacc-2026-site"
CHUNK_FILE = os.path.join(ROOT, "Stacc/blog-migration/small-chunks/chunk-016.txt")
PROGRESS_FILE = os.path.join(ROOT, "Stacc/blog-migration/progress.json")
CHUNK_PROGRESS_FILE = os.path.join(ROOT, "Stacc/blog-migration/small-chunks/chunk-016.txt.progress.json")
...
CATEGORY_KEYWORDS = [
    ("AI & Emerging", ["ai", "agent", "agents", "chatgpt", "gpt", "llm", "generative", "gemini", "perplexity", "claude", "copilot", "machine learning", "algorithm update", "geo", "generative-engine", "ai search", "ai-overview", "ai mode", "humanize", "human-ai", "ai overviews"]),
    ("Local SEO", ["local seo", "local business", "google business profile", "gbp", "map pack", "near me", "citation", "dentist", "lawyer", "contractor", "realtor", "salon", "florist", "electrician", "funeral", "gym", "hvac", "chiropractor", "accountant", "home service", "restaurant", "google maps ranking", "maps ranking"]),
    ("Content Strategy", ["content strategy", "content marketing", "editorial", "calendar", "blog post", "writing", "brief", "newsletter", "email marketing", "email automation", "subject line", "segmentation", "ugc", "evergreen", "format", "fact-check", "eeat", "e-e-a-t", "content writer", "outsource content", "inbound marketing", "content team"]),
]
...
def classify_author(title, slug, category):
    ...
    ritik = ["growth", "programmatic", "cro", "analytics", "ga4", "search console", "conversion", "funnel", "ab test", "a/b test", "roi", "statistics", "case study", "data", "traffic"]
    s = sum(1 for k in sidd if k in text)
    r = sum(1 for k in ritik if k in text)
    if category == "AI & Emerging":
        s += 2
    if category == "Local SEO":
        # growth/local optimization -> Ritik, otherwise Akshay
        if any(k in text for k in ["ranking", "maps", "citation", "conversion"]):
            r += 1
    if s > r and s > 0:
...
ALLOWED_TAGS = {"p", "ul", "ol", "li", "pre", "code", "table", "thead", "tbody", "tr", "th", "td", "strong", "em", "br", "span", "h3", "h4", "a"}

def clean_element(el):
    for tag in list(el.find_all(["script", "style", "img", "iframe", "svg", "button", "form", "input", "nav", "footer", "header"])):
        tag.decompose()
...
def parse_source(html_text):
    ...
    if col:
        # remove noisy end sections
        for h2 in col.find_all("h2"):
            txt = h2.get_text(strip=True).lower()
            if any(x in txt for x in ["explore more", "stop writing", "get your free", "check your inbox", "subscribe", "newsletter", "related posts", "you may also"]):
        ...
        for idx, h in enumerate(headings):
            if re.search(r"\bfaq\b|\bfrequently asked\b", h.get_text(strip=True), re.I):
                faq_title = None
                faq_body = []
...
def optimize_title(h1, slug):
    ...
    # remove trailing " | theStacc" if present
    t = re.sub(r"\s*\|\s*theStacc\s*$", "", t)
    if len(t) > 58:
        t = t[:55].rsplit(" ", 1)[0]
    t = f"{t} | theStacc"
    return t
...
def make_feature_svg(title, slug):
  ...
  <circle cx="580" cy="120" r="60" fill="{color2}" opacity="0.25"/>
  <rect x="60" y="160" width="120" height="80" rx="12" fill="{color1}" opacity="0.15"/>
  <path d="M40 220 L140 120 L240 220" stroke="{color1}" stroke-width="3" fill="none" opacity="0.4"/>
  <rect x="280" y="220" width="160" height="44" rx="22" fill="{color1}"/>
  ...
  <text x="360" y="175" text-anchor="middle" font-family="Geist, sans-serif" size="16" fill="#57534e">2026 Guide · Tactics · Examples</text>
...
def topic_noun(h1):
    return h1.split(":")[0].split("(")[0].strip().lower()
...
def build_sections_html(sections, category, slug, h1):
    ...
    for sec in sections:
        ...
        if body:
            ...
            parts.append(body)
    # inject contextual internal link near top if not already present
    joined = "\n".join(parts)
    if links and '<a href=' not in joined[:1200]:
        insert_idx = min(3, len(parts))
        parts.insert(insert_idx, f'<p>If you want to go deeper, explore our {links[0]} and {links[1]}.</p>')
        joined = "\n".join(parts)
    # ensure table
    if "<table>" not in joined.lower():
        noun = topic_noun(h1)
        parts.append(f'<h2 id="comparison">{html.escape(noun.title())} approaches compared</h2>')
        parts.append(f'<p>Not every approach to {noun} fits every site. Use this comparison to pick the right strategy for your resources and goals.</p>')
        parts.append('''<div class="table-wrap"><table>
<thead><tr><th>Approach</th><th>Best for</th><th>Time to results</th><th>Key risk</th></tr></thead>
<tbody>
<tr><td>Do-it-yourself</td><td>Founders and small teams</td><td>2–4 months</td><td>Inconsistent execution</td></tr>
<tr><td>In-house specialist</td><td>Growing companies with budget</td><td>1–3 months</td><td>Hiring and retention</td></tr>
<tr><td>Agency / platform</td><td>Scale and complex sites</td><td>30–60 days</td><td>Choosing the wrong partner</td></tr>
</tbody>
</table></div>''')
    # ensure ordered list
    if "<ol>" not in joined.lower():
        noun = topic_noun(h1)
        parts.append(f'<h2 id="steps">A simple {noun} process</h2>')
        parts.append(f'<p>Use this repeatable process to make {noun} a habit instead of a one-off project.</p>')
        parts.append('''<ol>
<li><strong>Audit the current state.</strong> Record your rankings, traffic, technical health, and content gaps before making changes.</li>
<li><strong>Prioritize by impact.</strong> Fix indexing and speed issues first, then target high-intent keywords with clear content upgrades.</li>
<li><strong>Execute in weekly sprints.</strong> Publish, update internal links, and monitor Search Console every week to catch regressions early.</li>
</ol>''')
    # ensure unordered list
    if "<ul>" not in joined.lower():
        noun = topic_noun(h1)
        parts.append(f'<h2 id="essentials">Core elements of {noun}</h2>')
        parts.append(f'<p>Focus on these fundamentals to avoid spreading effort too thin.</p>')
        parts.append('''<ul>
<li><strong>Clear intent matching.</strong> Every page should answer a specific question or solve a specific problem.</li>
<li><strong>Fast, crawlable pages.</strong> Technical issues block even the best content from ranking.</li>
<li><strong>Internal linking.</strong> Connect related pages so authority and users flow through your site.</li>
</ul>''')
    # common mistakes if missing
    joined = "\n".join(parts)
    if not any("common mistake" in p.lower() or "mistakes to avoid" in p.lower() for p in parts):
        ...
        parts.append(f'<p>Even experienced teams slip up when executing {topic_noun(h1)}. Here are the errors that waste budget and rankings.</p>')
        parts.append('''<ul>
<li><strong>Skipping the audit.</strong> Acting without a baseline makes it impossible to measure impact or prioritize fixes.</li>
<li><strong>Chasing vanity metrics.</strong> Traffic without intent or conversions does not grow revenue.</li>
<li><strong>Ignoring technical basics.</strong> Crawl errors, slow pages, and poor mobile experience undercut every content win.</li>
</ul>''')
    return "\n".join(parts), list(seen_ids)
...
def build_page(slug, data, progress):
    ...
    # lede from first body or generated
    ...
    if category in ("Local SEO", "Content Strategy", "SEO Tips"):
    ...
    if "common-mistakes" not in h2_ids:
    ...
    for slug in slugs:
        ...
        try:
            ...
            if not data["sections"]:
                state["failed"][slug] = "no content sections extracted"
                save_chunk_progress(state)
                continue
            ...
            progress["posts"][slug].update({
                "status": "done",
                "category": data["category"],
                "author": classify_author(data["h1"], slug, data["category"]),
